...
const LeftChat = ({refreshAccessToken, leftBookState, setLeftBookState, userId}) => {
    ...
    useEffect(() => {
        ...
        stompClient.connect({}, function (frame) {
            console.log(frame);
            stompClient.subscribe(`https://api.cleanbook.site/ws/sub/${chattingRoomId}`, function (chatMessage) {//구독
    ...
    const userChattingSubmitHandler = (event) => {
        ...
        const now = new Date();
        stompClient.send(`https://api.cleanbook.site/ws/pub/${chattingRoomId}`, {},
            JSON.stringify({
                userDto:{
                    userId: 5,
                    nickname: "testing",
